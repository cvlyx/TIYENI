...
export class SecureStorage {
  ...
  async setTokens(accessToken: string, refreshToken: string): Promise<void> {
    ...
    try {
      if (this.isSecureStorageAvailable) {
        await Keychain.setInternetCredentials('tiyeni_tokens', 'access_token', accessToken);
        await Keychain.setInternetCredentials('tiyeni_refresh', 'refresh_token', refreshToken);
      } else {
  ...
  async setUserData(userData: any): Promise<void> {
    try {
      ...
      if (this.isSecureStorageAvailable) {
        await Keychain.setInternetCredentials('tiyeni_user', 'user_data', userDataString);
      } else {
  ...
  async enableBiometric(): Promise<void> {
    try {
      ...
      if (this.isSecureStorageAvailable) {
        await Keychain.setInternetCredentials('tiyeni_biometric', 'enabled', 'true');
      } else {
  ...
  async authenticateWithBiometric(): Promise<boolean> {
    try {
      ...
      // For now, just check if biometric is available
      // In a real implementation, you'd trigger actual biometric prompt
      return true;
    } catch (error) {
